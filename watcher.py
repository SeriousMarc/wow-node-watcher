...
if roi_r > mon_w or roi_b > mon_h or roi_l < 0 or roi_t < 0:
        ...
        while True:
            ...
            if frame is not None:
                ...
                if pending_count != stable_count:
                    needed = (
                        HIT_FRAMES_TO_CONFIRM
                        if pending_count > stable_count
                        else MISS_FRAMES_TO_CLEAR
                    )
                    if frames_at_pending >= needed:
                        if pending_count > stable_count:
                            ...
                            for _ in range(new_symbols):
                                beep()
                            print(
                                f"DETECTED: {stable_count} -> {pending_count} "
                                f"(+{new_symbols}, score={score:.3f})"
                            )
                        else:
                ...
                if frame_count % FPS == 0:
                    print(
                        f"frames: {frame_count} | now: {count} "
                        f"| stable: {stable_count} | score: {score:.3f}"
                    )
